...
func testnetCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a liquidityapp testnet",
		Long: `testnet will create "v" number of directories and populate each with
necessary files (private validator, genesis, config, etc.).
Note, strict routability for addresses is turned off in the config file.
Example:
	liquidityd testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "liquidityd", "Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "liquidityd", "Home directory of the node's cli configuration")
...
func InitTestnet(
	...
	for i := 0; i < numValidators; i++ {
		...
		accStakingTokens := sdk.TokensFromConsensusPower(1000, sdk.DefaultPowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin("reservecoin1", accTokens),
			sdk.NewCoin("reservecoin2", accTokens),
