...
func (s *IntegrationTestSuite) TestNewSetPoolSwapFeeRateCmd() {
	val := s.network.Validators[0]
	...
	for _, tc := range testCases {
		...
		s.Run(tc.name, func() {
			cmd := cli.NewSetPoolSwapFeeRateCmd()
			clientCtx := val.ClientCtx
