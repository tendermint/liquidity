...
func (k Keeper) DepositLiquidityPool(ctx sdk.Context, msg types.BatchPoolDepositMsg, batch types.LiquidityPoolBatch) error {
	...
	lastReserveRatio := sdk.NewDecFromInt(reserveCoins[0].Amount).QuoTruncate(sdk.NewDecFromInt(reserveCoins[1].Amount))
	depositableAmount := coinB.Amount.ToDec().MulTruncate(lastReserveRatio).TruncateInt()
	...
	if coinA.Amount.LT(depositableAmount) {
		depositableAmountB = coinA.Amount.ToDec().QuoTruncate(lastReserveRatio).TruncateInt()
		...
		depositableAmountA = coinB.Amount.ToDec().MulTruncate(lastReserveRatio).TruncateInt()
...
func (k Keeper) WithdrawLiquidityPool(ctx sdk.Context, msg types.BatchPoolWithdrawMsg, batch types.LiquidityPoolBatch) error {
	...
	for _, reserveCoin := range reserveCoins {
		...
		withdrawAmt := reserveCoin.Amount.Mul(msg.Msg.PoolCoin.Amount).ToDec().MulTruncate(withdrawProportion).TruncateInt().Quo(totalSupply)
...
func (k Keeper) TransactAndRefundSwapLiquidityPool(ctx sdk.Context, batchMsgs []*types.BatchPoolSwapMsg,
	...
	//if err := k.bankKeeper.InputOutputCoins(ctx, inputs, outputs); err != nil {
	//	fmt.Println(batchMsgs)
	//	fmt.Println(matchResultMap)
...
// execute transact, refund, expire, send coins with escrow, update state by TransactAndRefundSwapLiquidityPool
func (k Keeper) TransactAndRefundSwapLiquidityPoolDec(ctx sdk.Context, batchMsgs []*types.BatchPoolSwapMsg,
	matchResultMap map[uint64]types.MatchResultDec, pool types.LiquidityPool, batchResult types.BatchResultDec) error {
		...
		if msgAfter, ok := matchResultMap[batchMsg.MsgIndex]; ok {
			...
			if batchMsg.RemainingOfferCoin.IsPositive() {
				...
				if !batchMsg.ToBeDeleted && batchMsg.OrderExpiryHeight <= ctx.BlockHeight() {
					...
					// Add swap exchanged coin fee to multisend
					inputs = append(inputs, banktypes.NewInput(poolReserveAcc,
						sdk.NewCoins(sdk.NewCoin(batchMsg.ExchangedOfferCoin.Denom, msgAfter.ExchangedCoinFeeAmt.TruncateInt()))))
					outputs = append(outputs, banktypes.NewOutput(poolReserveAcc,
						sdk.NewCoins(sdk.NewCoin(batchMsg.ExchangedOfferCoin.Denom, msgAfter.ExchangedCoinFeeAmt.TruncateInt()))))
